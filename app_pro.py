...
with st.sidebar:
    ...
    if st.button("文字を追加"):
        if msg:
            t_obj = {"type": "i-text", "text": msg, "fill": final_text_color, "fontSize": 36, "fontFamily": "ゴシック体", "originX": "left", "originY": "top"}
...
draw.rectangle([PADDING + guide_margin, PADDING + guide_margin, PADDING + CARD_W - guide_margin, PADDING + card_h - guide_margin], outline="red", width=2)

st.subheader("2. 写真・文字を配置してください")
canvas_result = st_canvas(
    fill_color="rgba(0,0,0,0)", background_image=canvas_bg_gui,
    initial_drawing={"objects": st.session_state.canvas_objects},
    height=canvas_h, width=CANVAS_W, drawing_mode="transform",
    key=f"pro_v7_{st.session_state.canvas_key}_{st.session_state.design}",
)
...
if st.button("✅ 完成画像を確定する", type="primary", use_container_width=True):
    if canvas_result.image_data is not None:
        ...
        final_merged = Image.alpha_composite(bg_img_base.convert("RGBA"), rgba)
